...
// --- Media-insights resilience helpers (MEO read-only build) ---
// Instagram supports different metrics per media product type (FEED photo/
// carousel vs REELS vs STORY), and Meta shifts the matrix over time (e.g.
// `impressions` was retired in 2025). For a fully headless monthly review over
// mixed content, one unsupported metric on one post must not fail the whole
// call. These helpers let meta_get_instagram_media_insights drop metrics the
// API rejects and retry, reporting what it dropped.

// Metrics broadly valid across FEED and REELS; the floor we fall back to when
// the API rejects a metric we cannot name from the error text.
const IG_METRIC_FLOOR = [
  "reach",
  "likes",
  "comments",
  "shares",
...
function isMetricError(error: unknown): boolean {
  if (error instanceof AxiosError && error.response) {
    const metaError = (error.response.data as { error?: { code?: number; message?: string } } | undefined)?.error;
    return metaError?.code === 100 && /metric/i.test(String(metaError?.message ?? ""));
  }
  return false;
}

// Returns which of `candidates` the API named as unsupported in the error.
function namedBadMetrics(error: unknown, candidates: string[]): string[] {
  if (!(error instanceof AxiosError) || !error.response) return [];
  const msg = String((error.response.data as { error?: { message?: string } } | undefined)?.error?.message ?? "");
  const found = new Set<string>();
  const phrase = msg.match(/support the ([a-z_]+) metric/i);
  if (phrase) found.add(phrase[1]);
  for (const c of candidates) {
    if (new RegExp(`\\b${c}\\b`).test(msg)) found.add(c);
  }
...
Args:
  ...
  - breakdown (string, optional): 'action_type' (for profile_activity) or 'story_navigation_action_type' (for navigation)`,
    ...
    async ({ media_id, metrics, breakdown, response_format }) => {
      ...
      // Retry loop: on an "unsupported metric" error, drop the offending
      // metric(s) and try again. Each pass removes >=1 metric or returns, so
      // it always terminates. This keeps a headless run over mixed media
      // (feed + reels + story) from failing on a single incompatible metric.
      for (let i = 0; i < 12 && attempt.length > 0; i++) {
        try {
          const params: Record<string, unknown> = { metric: attempt.join(",") };
          if (breakdown) params.breakdown = breakdown;
          data = await client.get<{ data: unknown[] }>(`/${media_id}/insights`, params);
          break;
        } catch (error) {
          if (!isMetricError(error)) return errorResult(error);
          let bad = namedBadMetrics(error, attempt);
          if (bad.length === 0) {
            // Meta rejected a metric but did not name it (indexed form).
            // Fall back to the known-good floor; give up if already at floor.
            const nonFloor = attempt.filter((m) => !IG_METRIC_FLOOR.includes(m as typeof IG_METRIC_FLOOR[number]));
            if (nonFloor.length === 0) return errorResult(error);
            bad = nonFloor;
          }
          dropped.push(...bad);
          attempt = attempt.filter((m) => !bad.includes(m));
        }
      }
          ...
          content: [
            {
              type: "text",
              text:
                `# Media Insights: \`${media_id}\`\n\n` +
                `_None of the requested metrics are supported for this media type._\n` +
      ...
      if (response_format === "json") {
        const payload = { ...data, _dropped_metrics: dropped };
      ...
      if (dropped.length > 0) {
        lines.push("", `_Skipped (unsupported for this media type): ${dropped.join(", ")}_`);
      }
      return { content: [{ type: "text", text: lines.join("\n") }] };
    }
  );
